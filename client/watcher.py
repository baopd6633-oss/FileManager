...
def start_monitor(path):
    ...
    watcher = FileWatcher()
    watcher.sender.monitored_path = path

    # Gửi danh sách file ban đầu lên Server sau khi đường dẫn đã được gán
    import threading
    thread = threading.Thread(target=watcher.sender.send_file_list, daemon=True)
    thread.start()
